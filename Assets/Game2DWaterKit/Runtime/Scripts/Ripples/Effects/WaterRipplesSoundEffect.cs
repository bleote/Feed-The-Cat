namespace Game2DWaterKit.Ripples.Effects
{
    ...
    public class WaterRipplesSoundEffect
    {
        ...
        private bool IsPoolCreated => _poolRoot != null;
        public AudioClip AudioClip { get => _audioClip;
            set => ChangeAudioClip(value);
        }
        public float AudioVolume { get => _audioVolume;
            set => ChangeAudioVolume(Mathf.Clamp01(value));
        }
        public bool CanExpandPool { get => _canExpandPool;
            set => _canExpandPool = value;
        }
        public int PoolSize { get => _poolSize;
            set => _poolSize = Mathf.Clamp(value, 0, int.MaxValue);
        }
        public float AudioPitch { get => _audioPitch;
            set => _audioPitch = Mathf.Clamp(value, -3f, 3f);
        }
        public float MaximumAudioPitch { get => _maximumAudioPitch;
            set => _maximumAudioPitch = Mathf.Clamp(value, -3f, 3f);
        }
        public float MinimumAudioPitch { get => _minimumAudioPitch;
            set => _minimumAudioPitch = Mathf.Clamp(value, -3f, 3f);
        }
        public bool IsUsingConstantAudioPitch { get => _isUsingConstantAudioPitch;
            set => _isUsingConstantAudioPitch = value;
        }
